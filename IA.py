...
def create_the_AI_training_dataset(file_in, file_out_pd, file_out_sk):
    ...
    df_sp = spark.read.options(header=True).csv(f'hdfs://localhost:19000/tomca/user/all_files/{file_in}')
    ...
    sparkDF.write.option("header", True).csv(f'hdfs://localhost:19000/tomca/user/all_files_spark/{file_out_sk}')


def train_model(spark):
    df_sp = spark.read.options(header=True,inferSchema=True).csv(f'hdfs://localhost:19000/tomca/user/all_files_spark/file_sport_for_IA.csv')
    df_sp.printSchema()
    df_sp.show()
...
def use_model(spark):
    df_sp = spark.read.options(header=True, inferSchema=True, delimiter=";").csv(f'hdfs://localhost:19000/tomca/user/file_for_test/test.csv')
    df_sp.printSchema()
    test = df_sp.toPandas()
    print(test)
    ...
    sparkDF.write.option("header", True).csv(f'hdfs://localhost:19000/tomca/user/file_for_test/reponse.csv')
